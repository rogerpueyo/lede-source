...
gre_setup() {
	...
	case "$mode" in
		gretapip)
			gre_generic_setup $cfg $mode $ipaddr $peeraddr "gretap-$cfg"
			;;
		*)
...
proto_gretap_setup() {
	...
	json_init
	json_add_string name "gretap-$cfg"
...
grev6_setup() {
	...
	case "$mode" in
		gretapip6)
			gre_generic_setup $cfg $mode $ip6addr $peer6addr "grev6tap-$cfg"
			;;
		*)
...
proto_grev6tap_setup() {
	...
	json_init
	json_add_string name "grev6tap-$cfg"
	json_add_boolean link-ext 0
	json_close_object
...
proto_gretap_teardown() {
	...
	gretap_generic_teardown "gretap-$cfg"
}
...
proto_grev6tap_teardown() {
	...
	gretap_generic_teardown "grev6tap-$cfg"
}
